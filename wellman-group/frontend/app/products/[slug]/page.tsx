...
import { ArrowLeft, ArrowRight, CheckCircle, MessageCircle, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white/90 backdrop-blur-sm rounded-3xl border border-white shadow-sm p-7">
      <h2 className="text-xl font-black mb-5" style={{ color: '#1A3A6B' }}>{title}</h2>
      {children}
    </div>
  );
}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {items.map(([k, v]) => (
        <div key={k} className="rounded-xl p-5" style={{ borderLeft: '3px solid #2060B0', background: '#F0F7FF' }}>
          ...
          <p className="text-base font-semibold" style={{ color: '#1A3A6B' }}>{v}</p>
        </div>
      ))}
    </div>
  );
...
function Pills({ items }: { items: string[] }) {
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item) => (
        <span key={item} className="px-3 py-1.5 text-sm font-semibold rounded-full" style={{ background: 'rgba(26,58,107,0.08)', color: '#1A3A6B' }}>
          {item}
        </span>
      ))}
    </div>
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} style={{ borderBottom: '1px solid rgba(26,58,107,0.05)' }}>
              <td className="py-3 pr-4 text-sm font-semibold" style={{ color: '#1A3A6B' }}>{row[0]}</td>
              {row.slice(1).map((cell, j) => (
                <td key={j} className="py-3 pr-4 text-sm" style={{ color: '#4B6A8F' }}>{cell}</td>
              ))}
            </tr>
          ))}
        </tbody>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
      ...
      <div className="md:col-span-2">
        <Card title="NABH Specifications">
          <p className="text-sm mb-5" style={{ color: '#4B6A8F' }}>As per NABH & JCI Standards — Class 100/ISO 5</p>
          <DataTable
          ...
          />
          <div className="mt-5">
            <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: '#2060B0' }}>NABH Guidelines</p>
            <CheckGrid items={[
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={name} className="flex items-start gap-2.5">
                <CheckCircle size={13} className="shrink-0 mt-0.5" style={{ color: '#2060B0' }} />
                <div>
                  <span className="text-sm font-semibold" style={{ color: '#1A3A6B' }}>{name} — </span>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
      ...
      <Card title="Medical Gas Outlets">
        <p className="text-sm mb-3" style={{ color: '#4B6A8F' }}>4 international standards available</p>
        <DataTable
      ...
      <div className="md:col-span-2">
        <Card title="Vacuum & Air Plant">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
              <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: '#2060B0' }}>Accessories</p>
              <div className="flex flex-wrap gap-2">
                {['Ward Vacuum Unit', 'BPC Flowmeter', 'OT Suction Trolley', 'MOX Regulator'].map((a) => (
                  <span key={a} className="px-3 py-1 text-sm rounded-lg" style={{ background: '#F0F4FA', color: '#4B6A8F', border: '1px solid rgba(26,58,107,0.1)' }}>{a}</span>
                ))}
              </div>
            </div>
          </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
      <Card title="ICU Environmental Standard">
        ...
        <p className="text-sm mt-4 leading-relaxed" style={{ color: '#4B6A8F' }}>
    ...
    return (
      <>
        ...
        <main className="pt-36 pb-28 text-center">
          <h1 className="text-5xl font-black mb-4" style={{ color: '#1A3A6B' }}>Product Not Found</h1>
          <p className="text-slate-500 mb-8">This product doesn't exist or has been removed.</p>
          <Link href="/products" className="inline-flex items-center gap-2 px-6 py-3 text-white font-semibold text-base rounded-full" style={{ background: '#1A3A6B' }}>
      ...
      <main>
        ...
        <section className="relative pt-36 pb-14 overflow-hidden">
          ...
          <div className="relative z-10 max-w-[1600px] mx-auto px-6 sm:px-10 lg:px-16">
            <Link href="/products" className="inline-flex items-center gap-1.5 text-base text-slate-500 transition-colors mb-8 hover:opacity-80" style={{ color: '#4B6A8F' }}>
              <ArrowLeft size={14} /> All Products
            </Link>
            <p className="text-xs font-black uppercase tracking-[0.25em] mb-3" style={{ color: '#2060B0' }}>Product</p>
            <h1 className="text-5xl sm:text-6xl font-black tracking-tight leading-tight mb-5" style={{ color: '#1A3A6B' }}>
              {product.title}
            </h1>
            <p className="text-lg leading-relaxed max-w-2xl" style={{ color: '#4B6A8F' }}>{product.short_desc}</p>
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2">
                ...
                {galleryImages.length > 0 && (
                  <div
                    className="mb-6 bg-white/90 backdrop-blur-sm rounded-3xl border border-white shadow-sm overflow-hidden"
                    onMouseEnter={() => setGalleryPaused(true)}
                    onMouseLeave={() => setGalleryPaused(false)}
                  >
                    <div className="relative w-full mx-auto h-[520px] bg-white flex items-center justify-center">
                      <img
                ...
                {/* About card */}
                <div className="bg-white/90 backdrop-blur-sm rounded-3xl border border-white shadow-sm p-7">
                  <h2 className="text-xl font-black mb-4" style={{ color: '#1A3A6B' }}>About This Product</h2>
                  {paragraphs.length > 0 ? (
                    ...
                    <>
                      <p className="text-base leading-relaxed mb-5" style={{ color: '#4B6A8F' }}>{product.short_desc}</p>
                      <CheckGrid items={[
                ...
                <div className="mt-8 rounded-3xl p-6 flex items-center justify-between gap-4" style={{ background: 'linear-gradient(135deg, rgba(26,58,107,0.06) 0%, rgba(32,96,176,0.08) 100%)', border: '1px solid rgba(32,96,176,0.12)' }}>
                  <div>
                    <p className="font-bold text-base" style={{ color: '#1A3A6B' }}>See Projects Using This Product</p>
                    <p className="text-sm mt-0.5" style={{ color: '#4B6A8F' }}>Browse our completed hospital projects</p>
                  </div>
                  <Link href="/projects" className="shrink-0 inline-flex items-center gap-1.5 px-4 py-2 text-white text-sm font-semibold rounded-full transition-opacity hover:opacity-90" style={{ background: 'linear-gradient(135deg, #1A3A6B 0%, #2060B0 100%)' }}>
              ...
              <div className="space-y-4 sticky top-28 self-start">
                ...
                <div className="rounded-2xl p-6 text-white" style={{ background: 'linear-gradient(135deg, #1A3A6B 0%, #2060B0 100%)' }}>
                  <p className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: 'rgba(183,215,240,0.9)' }}>Get a Quote</p>
                  <p className="text-base mb-5 leading-relaxed" style={{ color: 'rgba(255,255,255,0.8)' }}>
                  ...
                  <a
                    ...
                    className="flex items-center justify-center gap-2 w-full py-2.5 bg-white font-bold text-base rounded-xl transition-opacity hover:opacity-90"
                    style={{ color: '#1A3A6B' }}
                  >
                  ...
                  <Link
                    ...
                    className="flex items-center justify-center gap-2 w-full py-2.5 mt-2 font-semibold text-base rounded-xl transition-colors hover:opacity-90"
                    style={{ background: 'rgba(255,255,255,0.15)', color: 'white' }}
                  >
                    Request Support
                  </Link>
                  <Link href="/contact" className="flex items-center justify-center gap-2 w-full py-2.5 mt-2 font-semibold text-base rounded-xl transition-colors" style={{ background: 'rgba(255,255,255,0.12)', color: 'white' }}>
                    Send Inquiry
                  </Link>
                  {products.length > 0 && (
                    <button
                      onClick={() => document.getElementById('products-tools')?.scrollIntoView({ behavior: 'smooth' })}
                      className="flex items-center justify-center gap-2 w-full py-2.5 mt-2 font-semibold text-base rounded-xl transition-colors"
                      style={{ background: 'rgba(255,255,255,0.15)', color: 'white' }}
                    >
                      View Products & Tools ↓
                    </button>
                ...
                <div className="bg-white/90 backdrop-blur-sm rounded-2xl border border-white shadow-sm p-5">
                  <p className="text-xs font-black uppercase tracking-widest mb-4" style={{ color: '#2060B0' }}>Other Products</p>
                  <div className="space-y-1">
                    {[
                      ...
                      <Link key={s.slug} href={`/products/${s.slug}`} className="flex items-center gap-2.5 py-2 text-base transition-colors rounded-lg px-2 hover:bg-[#F0F7FF]" style={{ color: '#4B6A8F' }}>
                  ...
                  {products.map((p) => (
                    <div key={p.id} className="rounded-2xl p-6 border border-slate-100 bg-white shadow-sm">
                      {p.image_url && (
                        <button
                          type="button"
                          onClick={() => setLightboxImg({ url: p.image_url!, alt: p.name })}
                          className="w-full rounded-xl mb-4 overflow-hidden bg-slate-50 flex items-center justify-center cursor-zoom-in"
                        >
                          <img src={p.image_url} alt={p.name} className="w-full h-auto max-h-72 object-contain" />
                        </button>
                      )}
                      <h3 className="font-bold text-lg mb-1.5" style={{ color: '#1A3A6B' }}>{p.name}</h3>
                      {p.description && <p className="text-base text-slate-500 leading-relaxed">{p.description}</p>}
                    </div>
                  ))}
                </div>
              </div>
